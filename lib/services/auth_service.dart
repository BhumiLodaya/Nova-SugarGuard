...
class AuthService {
  ...
  // --------------- GUEST MODE (Firebase stays dormant) ---------------

  /// Start an anonymous guest session — no Firebase calls at all.
  Future<AuthResult> guestLogin() async {
    try {
      final user = await _guest.createGuestUser();
      return AuthResult(
        success: true,
      ...
      return AuthResult(
        success: false,
        message: 'Failed to start guest session: $e',
      );
    }
  }

  /// Whether the current session is a guest (non-Firebase) session.
  Future<bool> isGuestSession() => _guest.isGuestMode();

  /// Whether onboarding has been completed for the guest.
  Future<bool> isGuestOnboarded() => _guest.isOnboardingComplete();
  ...
  bool isLoggedIn() {
    // Check local flag first (covers both guest and Firebase sessions)
    final local = _db.getSetting(AppConstants.keyIsLoggedIn, defaultValue: false);
    if (local is bool && local) return true;
